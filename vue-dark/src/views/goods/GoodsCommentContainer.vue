...
import { goodsComment, addGoodsComment } from "../../api/index.js";
export default {
  data() {
    return {
      goodsId: "",
  ...
  created() {
    this.goodsId = this.cid;
    this.getCommentByPage();
  },
  methods: {
    getCommentByPage() {
      goodsComment(this.goodsId, this.page).then(response => {
    ...
    postMsg() {
      ...
      commentData.append("goods", this.cid);
      addGoodsComment(commentData).then(response => {
        this.msg = "";
        this.getCommentByPage();
      });
    }
