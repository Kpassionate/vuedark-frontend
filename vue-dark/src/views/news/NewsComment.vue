...
import { newsComment, addNewsComment } from "../../api/index.js";
export default {
  data() {
    return {
      newsId: "",
  ...
  created() {
    this.newsId = this.cid;
    this.getCommentByPage();
  },
  methods: {
    getCommentByPage() {
      newsComment(this.newsId, this.page).then(response => {
    ...
    postMsg() {
      ...
      commentData.append("news", this.cid);
      addNewsComment(commentData).then(response => {
        this.msg = "";
        this.getCommentByPage();
      });
    }
